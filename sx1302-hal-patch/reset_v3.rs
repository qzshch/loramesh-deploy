...
pub fn reset() -> Result<()> {
    ...
    if sx130x.is_some() {
        ...
        sleep(Duration::from_millis(100));
        // Release reset: SX130x reset is active-low, must end HIGH (Active)
        // so the chip exits reset and operates normally. Without this, the pin
        // stays LOW → SX130x held in permanent reset → TX fails (Bug #49).
        sx130x.set_lone_value(gpiocdev::line::Value::Active)?;
        sleep(Duration::from_millis(100));
    }
